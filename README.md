...
**⚠️ 安全提示**: 首次登录后请立即修改管理员密码！生产环境中务必使用强密码。
...
- **管理员 (admin)**: 拥有全部权限，可访问所有管理功能
- **裁判长 (chief_judge)**: 可访问管理员分配的赛项及其下所有数据，包括赛事管理、赛项管理、模块管理、评分管理等
- **裁判 (judge)**: 仅能访问评分管理模块，且只能查看已结束状态的模块，对分配给自己的选手进行评分
- **选手 (contestant)**: 专用简化界面，直接进入进行中的模块详情页面，只能查看赛题和上传答题附件

## 用户界面说明

### 管理员界面
- 完整的管理界面，包含所有功能模块
- 左侧导航菜单：用户管理、赛事管理、赛项管理、模块管理、评分管理

### 裁判长界面
- 管理界面，包含除用户管理外的所有功能模块
- 左侧导航菜单：赛事管理、赛项管理、模块管理、评分管理
